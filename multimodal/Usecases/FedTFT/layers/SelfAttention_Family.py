...
try:
    ...
    def rearrange(x, pattern):
        # Minimal fallback for pattern used in this file.
        if pattern == 'b j -> b 1 1 j':
            return x.unsqueeze(1).unsqueeze(1)
        raise ModuleNotFoundError(
            "einops is required for rearrange pattern: {}".format(pattern)
        )
...
class ReformerLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        if LSHSelfAttention is None:
            raise ModuleNotFoundError(
                "reformer_pytorch is required for ReformerLayer but is not installed."
            )
