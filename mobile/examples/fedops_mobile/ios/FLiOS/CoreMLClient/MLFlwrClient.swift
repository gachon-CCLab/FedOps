...
public class MLFlwrClient: Client {
    ...
    private let log = Logger(subsystem: Bundle.main.bundleIdentifier ?? "FedOps.mobile",
                                    category: String(describing: MLFlwrClient.self))
    private var eventSink: FlutterEventSink?

       // Setter for eventSink
       func setEventSink(_ sink: FlutterEventSink?) {
           print("event sink is set")
           self.eventSink = sink
       }

       // Use eventSink when you need to send events
       private func sendEvent(message: String) {
           print("message need to be sent")
           print(eventSink.debugDescription)
           eventSink?(message)
       }
    ...
    private func runMLTask(configuration: MLModelConfiguration, task: MLTask) -> MLResult {
        ...
        let completionHandler: (MLUpdateContext) -> Void = { finalContext in
            ...
            DispatchQueue.main.async {
                self.sendEvent(message: "hello from here")
            }
...
//extension MLFlwrClient: FlutterStreamHandler {
