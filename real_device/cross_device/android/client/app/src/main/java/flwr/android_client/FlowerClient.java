...
int round = 1;
    ...
    public Pair<ByteBuffer[], Integer> fit(ByteBuffer[] weights, int epochs, int training_rounds, ProgressBar prBar) {

        this.local_epochs = epochs;
        tlModel.updateParameters(weights);
        isTraining.close();
        tlModel.train(this.local_epochs, training_rounds,  prBar);
    ...
    public void setLastLoss(int epoch, float newLoss) {
        if (epoch == this.local_epochs - 1) {
            ...
            tlModel.disableTraining();
            isTraining.open();
            Log.e(TAG, "current Epoch::"+epoch);

            mActivity.setRoundText();
